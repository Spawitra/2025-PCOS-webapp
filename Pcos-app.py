...
# ========== Main UI ==========
logo = load_image("สมพงษ์ มงคล เปี๊ยก.png")
st.image(logo, use_container_width=True)
# โหลดภาพโลโก้
# แสดงโลโก้แบบกำหนดขนาด
if logo:
    st.image(logo, width=120)  # 👈 ปรับขนาดตามที่ต้องการ (px)
if logo:
    st.markdown(
        f"""
        <div style="display:flex; justify-content:center; align-items:center;">
            <img src="data:image/png;base64,{base64.b64encode(open('สมพงษ์ มงคล เบ็ญจ.png', 'rb').read()).decode()}" width="120">
        </div>
        """,
        unsafe_allow_html=True
    )
st.title("🧬📝 Polycystic Ovary Syndrome (PCOS) Risk Self-Assessment")
st.markdown("### ประเมินความเสี่ยงโรคภาวะถุงน้ำในรังไข่หลายใบ ")
st.error("📍 หมายเหตุ: การกรอกข้อมูลเป็นเพียงการประเมินความเสี่ยงเบื้องต้น ไม่ใช่การวินิจฉัยทางการแพทย์")
...
with st.container():
    ...
    with col1:
        ...
        #st.error("หากผู้ประเมินมีประจำเดือนมามากกว่า 15วัน ควรปรึกษาแพทย์ผู้เชี่ยวชาญ")
        hair_growth = st.radio("Hair growth มีขนดกมากกว่าปกติหรือไม่?", ["Y", "N"])
        st.warning("⚠️ สังเกตว่าขนในจุดต่างๆ ของร่างกาย ⚠️ หากเดิมอยู่ในสภาพปกติ (เช่น ระดับ 1 หรือ 2) แต่ช่วงหลังๆ ระดับความหนาเพิ่มขึ้น 2-4 หรือไม่")
        if HairG:
